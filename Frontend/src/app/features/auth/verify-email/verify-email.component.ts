...
import { DevCodeService } from '../../../core/services/dev-code.service';
...
@Component({
  ...
  template: `
    <div class="auth-card">
      ...
      @if (devCode.enabled && devCode.message(); as msg) {
        <div class="dev-code-box">
          <mat-icon>science</mat-icon>
          <div class="dev-code-box-text">
            <strong>Dev-only verification code</strong>
            <p>{{ msg }}</p>
          </div>
        </div>
      }
  ...
  styles: [
    `
      ...
      .dev-code-box {
        ...
        mat-icon { color: var(--warning); font-size: 20px; height: 20px; width: 20px; margin-top: 1px; }
        strong { display: block; font-size: 12.5px; margin-bottom: 2px; }
...
export class VerifyEmailComponent implements OnInit {
  ...
  readonly devCode = inject(DevCodeService);
  ...
  ngOnInit(): void {
    this.prefillFromDevCode();
  }

  /** Dev-only: prefill the code returned by the backend in the response message. */
  private prefillFromDevCode(): void {
    const code = this.devCode.code();
  ...
  resend(): void {
    ...
    this.auth
      ...
      .subscribe({
        next: () => {
          this.toast.info('Code sent', `A fresh verification code was sent to ${this.email()}.`);
          this.prefillFromDevCode();
        },
      });
  }
